  ...
  <header class="bg-white shadow-sm sticky top-0 z-10">
    <div class="max-w-7xl mx-auto px-4 py-3 sm:px-6 lg:px-8 flex justify-between items-center">
      <h1 class="text-lg md:text-xl font-bold text-gray-900">
        <span class="text-blue-600">Edit</span> Data Anggota
      </h1>
  ...
  <div class="flex-1 overflow-y-auto">
    <main class="max-w-7xl mx-auto px-4 py-4 sm:px-6 lg:px-8">
      <div class="bg-white rounded-lg shadow-sm overflow-hidden mb-6">
        <form action="{{ route('admin.anggota.update', $anggota->id_user) }}" method="POST" enctype="multipart/form-data" class="space-y-4 p-4 sm:p-6">
          @csrf
          @method('PUT')
          ...
          <!-- Password Input -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Password (Biarkan kosong jika tidak ingin diubah)</label>
            <input type="password" name="password" 
                   class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-sm"
                   placeholder="Masukkan password baru (min. 6 karakter)" minlength="6">
          </div>
          ...
          <div>
            ...
            @if($anggota->foto_anggota)
              <div class="mb-4">
                <img src="{{ asset('storage/' . $anggota->foto_anggota) }}" alt="Current Image" class="max-h-40 rounded-md border">
              ...
              <div class="space-y-1 text-center">
                ...
                <div class="flex flex-col sm:flex-row justify-center text-sm text-gray-600">
                  <label class="relative cursor-pointer rounded-md font-medium text-blue-600 hover:text-blue-500">
                    ...
                    <input id="file-upload" name="foto_anggota" type="file" class="sr-only" accept="image/*">
                  </label>
