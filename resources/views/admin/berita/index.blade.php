@extends('layouts.app', ['title' => 'Berita - Admin'])
        ...
        <div class="container mx-auto px-6 py-8">
            <div class="flex items-center">
                <button class="text-white focus:outline-none bg-gray-600 px-4 py-2 shadow-sm rounded-md">
                    <a href="{{ route('admin.berita.create') }}">TAMBAH</a>
                </button>
                    ...
                    <table class="min-w-full table-auto">
                        <thead class="justify-between">
                            <tr class="bg-gray-600 w-full">
                                ...
                                <th class="px-16 py-2">
                                    <span class="text-white">LINK BERITA</span>
                                </th>
                                <th class="px-16 py-2">
                            ...
                            @forelse($beritas as $berita)
                                <tr class="border bg-white">
                                    <td class="px-16 py-2 flex justify-center">
                                        <img src="{{ $berita->image }}" class="w-10 h-100 object-fit-cover rounded-full">
                                    </td>
                                    <td class="px-16 py-2">
                                        {{ $berita->judul }}
                                    </td>
                                    ...
                                    <td class="px-16 py-2">
                                        <a href="{{ $berita->link_berita }}" target="_blank" class="text-blue-500">Lihat
                                            Berita</a>
                                    </td>
                                    <td class="px-10 py-2 text-center">
                                        <a href="{{ route('admin.berita.edit', $berita->id) }}"
                                            class="bg-indigo-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">EDIT</a>
                                        <button onClick="destroy(this.id)" id="{{ $berita->id }}"
